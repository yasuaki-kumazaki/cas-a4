...
def run_tournament(
    strategies,
    cfg: MatchConfig,
    play_both_orders: bool = True,  # A vs B and B vs A (reduces “first player” bias)
    ...
    for S1, S2 in combinations(strategies, 2):
        # fresh instances for each match (avoids state carry-over bugs)
        A = type(S1)()
        B = type(S2)()
        ...
        if play_both_orders:
            ...
            # note: recorded as (S1 vs S2) again, but from reversed starting order
...
def print_leaderboard(stats: Dict[str, Stats]):
    ...
    for name, st in stats.items():
        rows.append(
            (name, st.wins, st.draws, st.losses, st.matches, st.points_for, st.points_against, st.diff)
        )

    rows.sort(key=lambda r: (r[1], r[7], r[5]), reverse=True)  # wins, diff, points_for
...
def main():
    cfg = MatchConfig(N=100, rounds=1000, verbose=False)  # verbose only for single matches; tournament forces verbose=False
    strategies = make_strategies()

    stats, h2h = run_tournament(strategies, cfg, play_both_orders=True)
    print_leaderboard(stats)

    # Optional: print head-to-head totals
    print("\n=== Head-to-Head (totals over the pair) ===")
    for (a, b), (sa, sb) in sorted(h2h.items()):
        print(f"{a:<18} vs {b:<18} -> {sa:.2f} : {sb:.2f}")
